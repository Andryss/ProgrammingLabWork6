...
public class FilterByMpaaRatingCommand extends NameableCommand {
    ...
    public boolean execute(ServerExecutor.ExecuteState state, ServerINFO server) throws CommandException {
        ...
        println("Found movies with \"" + mpaaRating + "\" mpaa rating:");
        List<Map.Entry<Integer,Movie>> found = server.getCollection().entrySet().stream()
                .filter(entry -> entry.getValue().getMpaaRating() == mpaaRating)
                .collect(Collectors.toList());
        if (found.size() == 0) {
            println("*nothing*");
        } else {
            found.forEach(entry -> println(entry.getKey() + " - " + entry.getValue()));
        }
        return true;
    }
